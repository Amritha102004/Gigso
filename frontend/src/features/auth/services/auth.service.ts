...
export const authService = {
  sendOtp: async (data: SignupPayload): Promise<SignupOtpResponse> => {
    const response = await authApi.post<ApiResponse<unknown>>(AUTH_ROUTES.SIGNUP, data);
    return { message: response.data.message };
  },

  verifyOtp: async (data: OtpPayload): Promise<VerifyOtpResponse> => {
    const response = await authApi.post<ApiResponse<{ user?: UserDTO }>>(AUTH_ROUTES.VERIFY_OTP, data);
  ...
  resendOtp: async (data: ResendOtpPayload): Promise<SignupOtpResponse> => {
    const response = await authApi.post<ApiResponse<unknown>>(AUTH_ROUTES.RESEND_OTP, data);
    return { message: response.data.message };
  },

  login: async (data: LoginPayload): Promise<AuthResponse> => {
    const response = await authApi.post<ApiResponse<{ user: UserDTO; accessToken: string }>>(AUTH_ROUTES.LOGIN, data);
    return { message: response.data.message, ...response.data.data! };
  },

  googleLogin: async (data: GoogleLoginPayload): Promise<AuthResponse> => {
    const response = await authApi.post<ApiResponse<{ user: UserDTO; accessToken: string; requiresRole?: boolean }>>(AUTH_ROUTES.GOOGLE_LOGIN, data);
    return { message: response.data.message, ...response.data.data! };
  },

  forgotPassword: async (data: ForgotPasswordPayload): Promise<SignupOtpResponse> => {
    const response = await authApi.post<ApiResponse<unknown>>(AUTH_ROUTES.FORGOT_PASSWORD, data);
    return { message: response.data.message };
  },

  resetPassword: async (data: ResetPasswordPayload): Promise<SignupOtpResponse> => {
    const response = await authApi.post<ApiResponse<unknown>>(AUTH_ROUTES.RESET_PASSWORD, data);
    return { message: response.data.message };
  },

  changePassword: async (data: ChangePasswordPayload): Promise<SignupOtpResponse> => {
    const response = await authApi.post<ApiResponse<unknown>>(AUTH_ROUTES.CHANGE_PASSWORD, data);
    return { message: response.data.message };
  },
